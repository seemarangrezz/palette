...
document.addEventListener("DOMContentLoaded", async () => {
  /* Content now lives in a database (or this browser), not in the markup —
     so every page waits for one fast read before it paints. */
  await Store.boot();
...
const KEY_ICON = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="4" y="10" width="16" height="10" rx="1.6"/><path d="M8 10V7a4 4 0 0 1 8 0v3"/></svg>`;
...
function injectNav(site) {
  ...
  /* The artist's own way in. Deliberately quiet — a hairline-bordered key
     rather than a button that shouts — so it reads as part of the gallery
     furniture to a visitor, but is unmissable to the person who owns it. */
  const adminLink = `<a href="admin.html" class="nav-admin" title="Sign in to manage the site">${KEY_ICON}<span>Admin</span></a>`;
  ...
  mount.innerHTML = `
    ...
    <div class="nav-mobile" id="navMobile">${linkHTML()}${adminLink}</div>
  `;
}
  ...
  mount.innerHTML = `
    <footer>
      ...
      <div class="footer-bottom">
        ...
        <a href="admin.html" class="footer-admin">Studio Admin — Sign In</a>
      </div>
    </footer>
  `;
}
